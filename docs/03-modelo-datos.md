...
-- La calidad del dato es propiedad de la meteo asignada, NO del FWI; el endpoint del
-- doc 06 sirve calidadDato desde aquí. Con fuente en REJILLA (reanálisis, ADR-07) no hay
-- estaciones: la calidad es la MAGNITUD DEL DOWNSCALING altitudinal (elevacion_celda_m,
-- delta_altitud_m). A mayor |delta|, menos fiable. (V8 sustituyó interpolado/n_estaciones.)
create table meteo_municipio (
  ...
  elevacion_celda_m numeric(7,2) not null,          -- cota nativa del modelo en el punto
  delta_altitud_m   numeric(7,2) not null,          -- altitud_media_m - elevacion_celda_m
  source            text     not null,
  source_url        text,                           -- regla de procedencia (doc 02)
  fetched_at        timestamptz not null default now(),
